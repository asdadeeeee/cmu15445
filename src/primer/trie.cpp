...
template <class T>
auto PutNode(const std::shared_ptr<const TrieNode>& old_node, std::string_view key, T value) -> std::shared_ptr<const TrieNode>{
...
auto RemoveNode(const std::shared_ptr<const TrieNode>& old_node, std::string_view key) -> std::shared_ptr<const TrieNode>{
  std::shared_ptr<TrieNode> new_node = nullptr;
  //如果键空，则该node为所寻node
  if(key.empty())
  {
